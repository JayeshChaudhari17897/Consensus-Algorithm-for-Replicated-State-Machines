...
class Node:
    ...
    def is_valid_event(self, h, event: Event):
        ...
        return (event.sha512 == h and (event.parents == () or (
        len(event.parents) == 2 and event.parents[0] in self.hg and event.parents[1] in self.hg and self.hg[
            event.parents[0]].verify_key == event.verify_key and self.hg[
            event.parents[1]].verify_key != event.verify_key)))
